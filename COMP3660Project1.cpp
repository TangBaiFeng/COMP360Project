...
void topDownParser() {
    ...
    if (lexicalReturn.at(2).first != '(') error(lexicalReturn.at(2).second);
    ...
    }    if (lexicalReturn.at(5).first != ')') error(lexicalReturn.at(5).second);
    if (lexicalReturn.at(6).first != '{') error(lexicalReturn.at(6).second);
    ...
    while (declare) {   // Checks if all initialized variables are valid. 
        ...
        if (lexicalReturn.at(counter + 2).first != ';') error(lexicalReturn.at(counter + 2).second);
    ...
    if (lexicalReturn.at(counter).first != ';') error(lexicalReturn.at(counter).second);
    counter++;
    if (lexicalReturn.at(counter).first != '}') error(lexicalReturn.at(counter).second);
